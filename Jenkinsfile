...
pipeline {
    ...
    stages {
        ...
        stage('Checkout Cloud-Native SG Git Repository') {
            steps {
                git branch: 'master',
                        url: '${GERRIT_MIRROR}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-enmsg-ebs-topology'
                  sh '''
                      git remote set-url origin --push ${GERRIT_CENTRAL}/OSS/ENM-Parent/SQ-Gate/com.ericsson.oss.containerisation/eric-enmsg-ebs-topology
                  '''
            }
        }
        stage('Helm Dep Up ') {
        ...
        stage('Merge values files') {
            steps{
                script {
                    appconfig_values = sh (script: "ls ${WORKSPACE}/chart/eric-enmsg-ebs-topology/appconfig/ | grep values.yaml", returnStatus: true)
                    ...
                    sh '''
                         if git status | grep 'values.yaml' > /dev/null; then
                            git add chart/eric-enmsg-ebs-topology/values.yaml
